...
/// Compute per-pixel star contribution and *add* it into the
/// existing RGBA buffer (which already holds the density-field glow).
#[spirv(compute(threads(8, 8, 1)))]
pub fn render_stars(
    #[spirv(global_invocation_id)] id: UVec3,
    #[spirv(uniform, descriptor_set = 0, binding = 0)] params: &GalaxyUniform,
    ...
    // World-space (x,z) at pixel centre.
    ...
    // Seed the RNG from *world* position (quantized) so stars
    // follow the galaxy when panning, rather than sticking to screen pixels.
    // Quantize to ~0.1 LY cells; add a large offset to keep coords positive
    // (float → uint conversion saturates negatives to 0).
    ...
    let n_stars = poisson(lambda, &mut rng);

    if n_stars == 0 {
        return;
    }

    // Accumulate star light.
    ...
    for _ in 0..n_stars {
        ...
        let rgb = temperature_to_rgb(temp) * lum;
        acc_r += rgb.x;
        acc_g += rgb.y;
        acc_b += rgb.z;
    }

    // Map luminosity to 8-bit and add to the existing RGBA.
    // Scale factor tuned so a 1 L☉ star at reference distance is clearly visible.
    let scale: f32 = 0.15;

    let dr = (acc_r * scale * 255.0).min(255.0) as u32;
    let dg = (acc_g * scale * 255.0).min(255.0) as u32;
    let db = (acc_b * scale * 255.0).min(255.0) as u32;

    let pixel_old = rgba[idx];
    let r_old = (pixel_old >> 16) & 0xff;
    let g_old = (pixel_old >> 8) & 0xff;
    let b_old = pixel_old & 0xff;

    let r_new = (r_old + dr).min(255);
    let g_new = (g_old + dg).min(255);
    let b_new = (b_old + db).min(255);

    rgba[idx] = r_new << 16 | g_new << 8 | b_new | 0xff_00_00_00;
}
